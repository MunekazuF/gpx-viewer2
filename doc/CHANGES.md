...
-   標高グラフの表示モードを「標高」「標高差」「獲得標高」に切り替える機能を追加
    -   グラフ内左上のトグルボタンで3つのモードを順に切り替えられるようにした
...
-   サイドバーで同じファイル名をクリックした場合でも、その都度地図がズーム＆移動するように改善
-   PC表示のサイドバーの初期幅を調整

### 修正 (Fixed)

-   グラフのサイズ変更機能に起因する、アプリケーションがクラッシュする、あるいは表示が崩れるといった複数の重大なバグを修正
...
-   フィルター処理を高速化
    -   フィルター適用時に全GPXデータを読み込むのではなく、メモリ上の軽量なメタデータのみで絞り込みを行うようにリファクタリングし、応答性を大幅に向上させた
...
-   初期表示時に地図が部分的にしか表示されない問題を修正
    -   `Split.js`の初期化前に地図コンテナが適切な高さを持つように、CSSの`.split-vertical`に`flex-grow: 1`を追加した
-   モバイル表示のレイアウト安定性を向上
    -   高さの単位を`%`から`dvh`に変更
    -   不要な`height`指定を削除
